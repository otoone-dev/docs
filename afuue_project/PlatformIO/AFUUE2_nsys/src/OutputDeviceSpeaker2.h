...
/*
    LEDC.channel_group[0].channel[LEDC_CHANNEL_1].duty.duty = waveLow;
    LEDC.channel_group[0].channel[LEDC_CHANNEL_2].duty.duty = waveHigh;
    LEDC.channel_group[0].channel[LEDC_CHANNEL_1].conf1.duty_start = 1;
    LEDC.channel_group[0].channel[LEDC_CHANNEL_2].conf1.duty_start = 1;
*/
#else
    ledcWrite(PWMPIN_LOW, waveLow);
    ledcWrite(PWMPIN_HIGH, waveHigh);
#endif
#else
    //dacWrite(DACPIN, h);
#endif
...
*/
#else
        ledcAttach(PWMPIN_LOW, 156250, 8); // PWM 156,250Hz, 8Bit(256段階)
        ledcWrite(PWMPIN_LOW, 0);
        ledcAttach(PWMPIN_HIGH, 156250, 8); // PWM 156,250Hz, 8Bit(256段階)
        for (int i = 0; i < 0x80; i++) {
            ledcWrite(PWMPIN_HIGH, i);
            delay(4);
        }
#endif
    ...
    OutputResult Update(float note, float vol) override {
        ...
        return OutputResult{ true, (float)cpuLoad };
    }
    ...
    static void CreateWaveTask(void *parameter) {
        ...
        while (1) {
            ...
            while (1) {
                ...
                float f = (fmid*1.8f) * (volume * (-0.5f + phase));
                //float f = (fmid*0.9f)* (volume * sinf(phase * 2.0f * 3.14159265f));
