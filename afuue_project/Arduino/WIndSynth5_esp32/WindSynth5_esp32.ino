...
static int noteBuffer[NOTE_BUFFER_SIZE];
...
void loop() {
  ...
  if (initCnt > 0) {
    ...
    ftt = CalcInvFrequency(currentNote + pitch);
     ...
     noteBuffer[notePos] = note;
     notePos = (notePos + 1) % NOTE_BUFFER_SIZE;
     bool updateFlag = true;
     for (int i = 1; i < NOTE_BUFFER_SIZE; i++) {
       if (noteBuffer[0] != noteBuffer[i]) {
         updateFlag = false;
         break;
       }
     }
...
uint8_t CreateWave() {
    ...
    // 波形を生成する
    double e = (127 * Voice(phase) * volReq) + reverbBuffer[reverbPos];// + phase2 * volReq2 + phase3 * volReq3;
    if ( (e < 0.2) && (e > -0.2) ) {
      e = 0.0;
    }
...
void setup() {
  ...
  if ((keyLowCs == HIGH) && (keyGs == HIGH) && (keyG == HIGH) && (keyA == HIGH) && (keyB == LOW)) {
    ...
    for (int i = 0; i < 256; i++) wave[i] = waveOboe[i];
    for (int i = 0; i < 256; i++) waveA[i] = waveOboeA[i];
  }
