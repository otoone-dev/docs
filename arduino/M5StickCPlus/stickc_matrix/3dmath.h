...
public:
  ...
  // 3D座標を2Dスクリーン座標に変換
  static Vector4 ProjectTo2D(const Vector4& point, const Matrix44& view, const Matrix44& projection, float screenWidth, float screenHeight) {
      // ワールド空間 -> ビュー空間
      Vector4 viewSpace = view * point;

      // ビュー空間 -> クリップ空間
      Vector4 clipSpace = projection * viewSpace;
